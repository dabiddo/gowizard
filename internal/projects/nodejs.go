...
func CreateHonoJsProject(name string) string {
	utils.ClearScreen()
	...
	fmt.Println("Creating HonoJs project...")

	// Run the command
	if err := cmd.Run(); err != nil {
		fmt.Printf("\nError creating NestJs project: %v\n", err)
		return "Failed to create HonoJs project"
	}
	...
	message := fmt.Sprintf("HonoJs project '%s' created successfully!\n", name)
	fmt.Print(message)
	return message
}

func CreateHonoOpenApi(name string) string {
	utils.ClearScreen()
	...
	message := fmt.Sprintf("Hono OpenAPI project '%s' created successfully!\n", name)
	fmt.Print(message)
	return message
}
