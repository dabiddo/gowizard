...
func CreateNuxtThreeProject(name string) string {
	utils.ClearScreen()
	...
	cmd := exec.Command("docker", "run", "--rm",
		...
		"dabiddo/larabox",
		"sh", "-c",
		fmt.Sprintf("pnpm create nuxt %s -t v3 --yes --package-manager pnpm --git-init && chown -R $(id -u):$(id -g) %s", name, name))
...
func CreateVueProject(name string) string {
	utils.ClearScreen()
	...
	cmd := exec.Command("docker", "run", "--rm",
		...
		"dabiddo/larabox",
		"sh", "-c",
		fmt.Sprintf("pnpm create vue@latest %s --default && chown -R $(id -u):$(id -g) %s", name, name))

	fmt.Println("Creating Vue project...")
	...
	// Run the command
	if err := cmd.Run(); err != nil {
		fmt.Printf("\nError creating Vue project: %v\n", err)
		return "Failed to create Vue project"
	}
	...
	message := fmt.Sprintf("Vue project '%s' created successfully!\n", name)
	fmt.Print(message)

	return "Created Vue Project"
}
